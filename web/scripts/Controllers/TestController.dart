...
void  monsterPartChecker() {
    MonsterGirlDoll selectedDoll = new MonsterGirlDoll();
    for(SpriteLayer layer in selectedDoll.dataOrderLayers) {
        for(int i = 0; i<layer.maxImageNumber+1; i++) {
            drawPart(selectedDoll,layer, i);
        }
    }
}
...
void lamiaHornTest() {
    ...
    for(int i = 0; i<leftHorn.maxImageNumber+1; i++) {
        drawPartPair(selectedDoll,leftHorn,rightHorn, i);
    }
}


Future<Null> drawPartPair(Doll doll, SpriteLayer part1, SpriteLayer part2, int imgNumber) async{
...
Future<Null> drawPart(Doll doll, SpriteLayer part1, int imgNumber) async{
    ImageElement img1 = await Loader.getResource("${part1.imgNameBase}${imgNumber}.${part1.imgFormat}");
    ...
    me.style.display = "inline-block";
    me.append(visible);
    DivElement number = new DivElement()..text = "${part1.name}: $imgNumber";
    me.append(number);
    container.append(me);
